...
def build_merge_sql(logical_date, offset_days=0, direct_transfer=False, **context):
    ...
    if direct_transfer:
        ...
        context['ti'].xcom_push(key=f"direct_transfer_success_{offset_days}", value=True) # Indicate successful direct transfer
        ...
        """
        context['ti'].xcom_push(key=f"merge_success_{offset_days}", value=True) # Indicate successful merge
        context['ti'].xcom_push(key=f"sql_to_execute_{offset_days}", value=sql)
...
def daily_report(**context):
    ...
    successfully_merged_tables = []
    successfully_transferred_tables = []

    for i in range(1, LOOKBACK_DAYS + 1):
        table_date = (logical_date - timedelta(days=i)).strftime("%Y-%m-%d")
        table_name = generate_table_name(TABLE_PREFIX, table_date)

        direct_transfer_successful = ti.xcom_pull(task_ids=f"build_sql_{i}", key=f"direct_transfer_success_{i}")
        if direct_transfer_successful:
            successfully_transferred_tables.append(table_name)
            continue

        merge_successful = ti.xcom_pull(task_ids=f"build_sql_{i}", key=f"merge_success_{i}")
        if merge_successful:
            successfully_merged_tables.append(table_name)

    if successfully_transferred_tables:
        logger.info(f"Successfully transferred the following tables: {', '.join(successfully_transferred_tables)}")
    else:
        logger.info("No tables were successfully transferred in this run.")

    if successfully_merged_tables:
        logger.info(f"Successfully merged the following tables: {', '.join(successfully_merged_tables)}")
    else:
        logger.info("No tables were successfully merged in this run.")
