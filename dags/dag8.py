...
RETRYABLE_ERRORS = (ServiceUnavailable, InternalServerError, TooManyRequests, GoogleCloudError)
...
def get_table_last_modified_time(bq_client, project_id, dataset_id, table_id):
    try:
        table = bq_retry(lambda: bq_client.get_table(f"{project_id}.{dataset_id}.{table_id}"))
