...
def schema_fingerprint(table):
    return "|".join([f"{f.name}:{f.field_type}" for f in table.schema])


def get_last_modified(table):
    return table.modified if table else None
...
def build_sql(logical_date, offset_days, direct_transfer=False, **context):
    ...
    source_schema = schema_fingerprint(source_table)
    source_last_modified = get_last_modified(source_table)
    ...
    target_schema = schema_fingerprint(target_table)
    target_last_modified = get_last_modified(target_table)
    ...
    data_changed = (
        target_last_modified is None
        or source_last_modified > target_last_modified
    )
...
with DAG(
    dag_id="bq_metadata_replication_final_report_fix",
